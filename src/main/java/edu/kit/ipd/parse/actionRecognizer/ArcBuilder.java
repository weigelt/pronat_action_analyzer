...
public class ArcBuilder {
     ...
     * instructions and instructions are assigned to predicates. Adds arcs
     * betwenn tokens. The arcs are type of "betweenToken" The arcs have an
     * attribute "betweenRole", which can be between predicates (NEXT_ACTION),
     * from predicates to actors/ parameters (PREDICATE_TO), or inside an actor/
     * parameter (INSIDE_CHUNK)
     */
    public void execute() {
        LinkedList<Predicate> predicates = this.actionGraph.getPredicates();
     ...
     * 
     * @param chunk
     *            a list of nodes containing a chunk. The chunk should play
     *            exactly ONE role.
     */
     ...
     * "PREDICATE_TO_PARA" arc starts at the beginning of the predicate and ends
    ...
    /**
     * Adds "NEXT_ACTION" Arcs which combine predicates. A "NEXT_ACTION" arc
     * starts at the beginning of the preceding predicate and ends at the
     * beginning of the succeeding predicate.
     * 
     * @param predicates
    ...
    private void addNextActionArcs(LinkedList<Predicate> predicates) {
        for (int i = 0; i < predicates.size() - 1; i++) {
            if (predicates.get(i).getTokens().size() > 0 && predicates.get(i + 1).getTokens().size() > 0) {
                IArc arc = actionGraph.createArc(predicates.get(i).getTokens().getFirst(),
                        predicates.get(i + 1).getTokens().getFirst(),
                        actionGraph.getArcType(props.getProperty("BETWEENTOKEN_ARC_TYPE")));
