...
public class ActionGraph extends AdapterGraph {
	...
	public ActionGraph(IGraph graph) {
      ...
      addNodeArcAttributes();
	}

	/**
	 * Initializes the tokens (a linkedList) with the nodes of the graph. The order
	 * is determined by the "NEXT" arc.
	 */
	private void initTokens() {
      ...
      tokens.add(currentNode);
		for (int i = 0; i < nodeCount - 1; i++) {
			IArc nextArc = currentNode.getOutgoingArcsOfType(graph.getArcType(props.getProperty("RELATION_ARC_TYPE"))).iterator().next();
	...
	private void initActions() {
      actions = new LinkedList<Action>();
        ...
        actions.get(instructionNr).addToken(token);
		}
	}

	/**
	 * Adds an attribute "role" to the node type "token", which is the type of Enum
	 * "Role", containing 6 roles. Adds a node type "newPredicate". Then adds
	 * attribute "value" to it, which is a string. Adds an arc type "betweenToken",
	 * then adds an attribute "betweenRole" to it, which is the type of Enum
	 * "BetweenRole", containing 3 betweenRole types.
	 *
	 */
	...
	/**
	 * Gets predicates in the graph. Also the new added predicates because of "and".
	 * 
	 * @return
	 */
